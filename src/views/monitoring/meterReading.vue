    ...
    <div class="content">
      <el-form ref="form" :model="form" :inline="true" label-width="100px" size="mini">
        <el-form-item label="集中器地址">
          <el-input v-model="form.vcaddr_web"/>
        </el-form-item>
        <el-form-item label="pn值">
          <el-input v-model.number="form.addr_web"/>
        </el-form-item>
        ...
        <el-form-item>
          <el-button type="primary" size="mini" @click="query(0)">查询</el-button>
      ...
      <el-table
        ...
        border
        style="width: 100%">
        <el-table-column prop="freeze_time" label="冻结时间" width="150"/>
        <el-table-column prop="vcaddr_web" label="集中器地址" width="150"/>
        <el-table-column prop="pn" label="pn" width="150"/>
        <el-table-column prop="tasktype" label="任务模板" />
        <el-table-column prop="data_type" label="数据类型" width="180"/>
        <!-- <el-table-column prop="" label="上一日用电量" width="120"></el-table-column> -->
        <el-table-column prop="forward_power_total" label="正向有功电能总" width="120"/>
        <el-table-column prop="forward_work_rate_1" label="正向有功费率1(尖)" width="120"/>
        <el-table-column prop="forward_work_rate_2" label="正向有功费率2(峰)" width="120"/>
        <el-table-column prop="forward_work_rate_3" label="正向有功费率3(平)" width="120"/>
        <el-table-column prop="forward_work_rate_4" label="正向有功费率4(谷)" width="120"/>
        <el-table-column prop="rid" label="任务轮次" width="120"/>
        <el-table-column prop="gather_time" label="采集时间" width="190"/>
      </el-table>
      <PagingQuery
        :pager="pager"
...
export default {
  ...
  methods: {
    ...
    // 获取采集时间
    collTime() {
      const type = this.form.data_type_value
    ...
    // 获取结束时间
    EndTime(time) {
      const end = new Date(time[1]).getTime() + 24 * 60 * 60 * 1000 - 1
      return end
    },
    // 重置
    reset() {
    ...
    // 隐藏总页数
    totalHide() {
      const total = document.getElementsByClassName('el-pagination__total')[0]
