<template>
  <div class="webscoket">
    <div>
      ...
      <el-button v-if="getconnect" type="success" @click="connect">连接</el-button>
      <el-button v-if="!getconnect" type="danger" @click="duankai">断开</el-button>
      <el-button v-if="!getconnect" type="info" @click="dingyue" >订阅</el-button>
    </div>
    <div class="center">
      <div class="header">
        <h3>LORA模块控制</h3>
    ...
    <el-dialog :visible.sync="webscoket" title="连接" width="30%" >
      <el-form ref="form" :model="webscoketForm">
        <div class="block">
          <div>
            <label for="">主机地址</label>
            <el-input v-model="webscoketForm.host" style="width:100%"/>
          </div>
          <div>
            <label for="">端口</label>
            <el-input v-model="webscoketForm.port" style="width:100%"/>
          </div>
          <div>
            <label for="">客户端ID</label>
            <el-input v-model="webscoketForm.clientId" style="width:100%"/>
          </div>
          <div>
            <label for="">用户名</label>
            <el-input v-model="webscoketForm.userName" style="width:100%"/>
          </div>
          <div>
            <label for="">密码</label>
            <el-input v-model="webscoketForm.password" style="width:100%" type="password"/>
          </div>
          <div>
            <label for="">心跳间隔</label>
            <el-input v-model="webscoketForm.keepAlive" style="width:100%" placeholder="KeepAlive"/>
          </div>
          <p>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="success" style="float:right" @click="connect">连接</el-button>
      </span>
    </el-dialog>
    <!--订阅窗口-->
    <el-dialog :visible.sync="dingyuedialog" title="连接" width="40%" >
      <div class="block1">
        <label for="">Topic:</label>
        <el-input v-model="topic" style="width:200px"/>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="info" style="float:right" @click="dingyuedialog = false">关 闭</el-button>
      </span>
    </el-dialog>
  </div>
</template>
