  ...
  <div class="appmarage">
    ...
    <div class="block" style="margin-top:10px;">
      <el-table :data="tableData.slice((start-1)*length,start*length)" style="width: 100%;text-align:center">
        ...
        <el-table-column :label="$t('plugins.version')" prop="version" align="center" sortable/>
        <el-table-column :label="$t('developer.describe')" prop="desc" align="center" sortable/>
        <el-table-column :label="$t('developer.operation')" align="right">
          ...
          <template slot-scope="scope">
            <el-button v-if="scope.row.active==false" type="info" size="small" plain @click="startup(scope.row.app)"><div style="width:10px;height:10px;border-radius:50%;display:inline-block;background:#00cc33;margin-right:10px"/>{{ $t('plugins.start') }}</el-button>
            <el-button v-if="scope.row.active==true" type="info" size="small" plain @click="stopup(scope.row.app)"><div style="width:10px;height:10px;border-radius:50%;display:inline-block;background:#a94442;margin-right:10px"/>{{ $t('plugins.stop') }}</el-button>
    ...
    <el-dialog
      ...
      <div>
        <el-table
          ...
          <el-table-column :label="$t('plugins.modulename')" prop="path" sortable align="center"/>
          <el-table-column :label="$t('plugins.ischange')" prop="is_changed" sortable align="center"/>
        </el-table>
        <div class="block">
          <el-pagination
            :page-sizes="[10, 25, 50, 100]"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button style="float:left" @click="dialogVisible = false">{{ $t('developer.cancel') }}</el-button>
