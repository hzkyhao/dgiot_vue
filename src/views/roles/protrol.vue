<template>
    <div class="protol">
       <div class="protolheader">
          <el-form :inline="true" :model="formInline" class="demo-form-inline" :rules="addRules" ref="formInline" size="small">
       ...
       <el-dialog title="协议管理" :visible.sync="dialogTableVisible" width="50%" :close-on-click-modal="false">
         <el-table :data="gridData" style="width:100%;text-align:center">
            ...
            <el-table-column  label="操作" align="center" width="300">
               ...
               <el-button type="success" size="mini" v-if="scope.row.attributes.data.enable==false" @click="kaisendable(scope.row)">启用</el-button>
               <el-button type="danger"  size="mini" v-if="scope.row.attributes.data.enable==true" @click="jiesendable(scope.row)">禁用</el-button>
               </template>
            </el-table-column>
         </el-table>
      </el-dialog>
     ...
     <pre id="editor" class="ace_editor" style="min-height:600px"><textarea class="ace_text-input"></textarea></pre>
...
export default {
   ...
   data() {
      ...
      let reg=/[0-9a-zA-Z]/
      if(!reg.test(value)){callback(new Error('协议名称由数字和字母组合'))
         }else{
            callback()
         }
      };
