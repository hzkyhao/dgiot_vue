  ...
  <div class="editdevices">
    ...
    <div>
      ...
      <!-- 右上角(图表,表格)的按钮 -->
            ...
            <div class="thirdtb" v-if="!isshowtable">
              ...
              <ul style="display: flex;flex-wrap: wrap">
                ...
                <li v-for="(item,index) in properties" :key="index" class="updatedtable">
                  ...
                  <div class="stla" :title="item.dataType.type" v-if="item.dataType.type=='double'||item.dataType.type=='float'||item.dataType.type=='int'">
                    <span>{{item.value | filterVal}}</span>
                    <span v-if="item.dataType.specs.unit">{{item.dataType.specs.unit}}</span>
                  </div>

                  <div class="stla" :title="item.dataType.type" v-if="item.dataType.type=='enmu'||item.dataType.type=='bool'">
                    <span>{{item.value | filterVal}}</span>
                    <span>{{item.dataType.specs[item.value]}}</span>
                  </div>

                  <div class="stla" :title="item.dataType.type" v-if="item.dataType.type=='struct'">
                    <i
                    ...
                    >
                      ...
                      <div class="stla" v-if="key.dataType.type=='enmu'||key.dataType.type=='bool'">
                        <span>{{key.name+':'}}</span>
                        <span>{{key.value}}</span>
                  ...
                  <div class="ta">
                    <el-link type="primary" :underline="false" @click="dataDetail(item)">查看数据</el-link>
                  </div>
                </li>
              </ul>
            </div>
        ...
        <el-tab-pane :label="$t('route.在线调试')" name="ninth"></el-tab-pane>
        <!--子设备管理-->
          ...
          >
            <div class="childdialog">
              <el-form ref="childrenForm" :model="childrenForm" class="demo-form-inline">
                ...
                >
                  <el-input v-model="childrenForm.route" placeholder="子网地址"></el-input>
                </el-form-item>
              </el-form>
            </div>
        ...
        <el-tab-pane label="设备指令" name="instruct" v-if="isshowchild">
          <Instruct :productId="productid" :devicesId="deviceid" />
        </el-tab-pane>
      </el-tabs>
      <!--data数据dialog-->
      <el-dialog
...
export default {
  ...
  methods: {
    ...
    makeSure(scope) {
      ...
      let objRoute = JSON.parse(JSON.stringify(scope.row.route))      

      let routeKey = this.devicedevaddr
      // 删除key为上级设备地址值
