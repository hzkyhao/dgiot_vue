      ...
      <el-tabs v-model="activeName" @tab-click="handleClick">
                ...
                <el-form ref="topicform" :model="topicform" :rules="topicrule">
                  ...
                  >
                    <el-select
                      ...
                      :placeholder="
                        $t('product.selectdeviceoperationpermission')
                      "
                    >
        ...
        <el-tab-pane :label="$t('product.physicalmodel')" name="third">
          <div style="text-align:right">
            ...
            >{{ $t("product.newcustomattribute") }}
            </el-button>
            <el-button
              type="primary"
              size="small"
              @click="addcategory"
            >{{ $t("product.newstandardattribute") }}
            </el-button>
          </div>
          <div>
            <el-table
                        ...
                        <el-col :span="10">
                          ...
                          >
                            ...
                            >
                              ...
                              />
                              <el-option
                                :label="$t('product.bool')"
                                value="bool"
                              />
                              <el-option
                                :label="$t('product.enum')"
                                value="enum"
                              />
                              <el-option
...
export default {
  ...
  methods: {
    ...
    addData() {
      ...
      this.$update_object('Channel', this.rproductdetailesourcechannelid, params).then(
        response => {
          if (response) {
            this.$message("添加成功");
          }
    ...
    menuTabClick(item, itemPos) {
      ...
      if (isAdd == false) {
        ...
        setTimeout(() => {
          ...
          this.editorList[this.editorList.length - 1].session.setMode("ace/mode/erlang"); // 设置语言
          this.editorList[this.editorList.length - 1].setTheme("ace/theme/monokai"); // 设置主题
          this.editorList[this.editorList.length - 1].setHighlightActiveLine(true);
    ...
    getProDetail() {
      ...
      this.$get_object("Product", this.productId).then(response => {
        ...
        if (response) {
          ...
          if (this.productdetail.thing == undefined || JSON.stringify(this.productdetail.thing) == "{}") {
