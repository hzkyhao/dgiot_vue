  ...
  >
    <div class="loginbanner">
      ...
      >
        ...
        <span
          style="float:left;color:#409EFF;font-size:14px;cursor:pointer;margin-right:20px;"
          @click="phonelogin"
        >手机号登录</span>
        <span
          style="float:right;color:#409EFF;font-size:14px;cursor:pointer"
  ...
  methods: {
    ...
    getTitle() {
      Sitepro(this.roleType).then(resultes => {
        ...
        sessionStorage.setItem(
          'dashboard',
          resultes.dashboard ? resultes.dashboard : '#'
        )
        sessionStorage.setItem('imgsrc', resultes.logo ? resultes.logo : '')
        sessionStorage.setItem(
          'copyright',
          resultes.copyright ? resultes.copyright : ''
        )
        ...
        if (valid) {
          ...
          this.$axiosWen
            ...
            .then(user => {
              ...
              sessionStorage.setItem(
                'roles',
                user.roles[0] ? user.roles[0].alias : ''
              )
              this.$Cookies.set(
                'appids',
